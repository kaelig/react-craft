...
- [x] **1.1** Initialize git repo (`git init`)
- [x] **1.2** Create directory structure (component dirs at plugin root, not inside `.claude-plugin/`):
  ```
  react-craft/
    .claude-plugin/
...
- [x] **1.3** Write `plugin.json`:
  ...
  ```
- [x] **1.4** Write MIT LICENSE file
- [x] **1.5** Write CHANGELOG.md (Unreleased section)
- [x] **1.6** Write plugin-level CLAUDE.md with shared values (condensed from charter):
...
- [x] **1.7** Write `skills/team-context/SKILL.md` with charter and roster as linked references
...
- [x] **1.8** Write `skills/templates/SKILL.md` with component-brief template and architecture template as references
- [x] **1.9** Write initial README with vision, install instructions, compatibility matrix
...
- [x] **2.0.1** Define sanitization module for Figma-sourced strings (restored from deepened plan CC.2):
...
- [x] **2.0.2** Define path canonicalization rule (restored from deepened plan CC.4):
...
- [x] **2.1** Create `commands/react-craft/init.md` with frontmatter:
  ```yaml
  ---
...
- [x] **2.2** Detection logic (package.json primary, fast):
...
- [x] **2.2.1** When multiple styling methods detected, prompt: "I found both Tailwind and styled-components. Which should react-craft use for new components?" Report file counts per method for context.

### Research Insights: tsconfig Parsing (TypeScript Review)

- [x] **2.2.2** Parse `tsconfig.json` for Code Writer:
...
- [x] **2.4.1** Display detected script commands and require user confirmation before adding to config. Malicious `package.json` scripts could exfiltrate data when Quality Gate runs them.

### Research Insights: Dependency Policy (Spec Flow Analysis)

- [x] **2.4.2** Ask about dependency policy: "Are there any UI libraries you want to avoid?" Store as `banned_dependencies` in config.

- [x] **2.5** Add CLAUDE.md instructions with `<!-- react-craft:start -->` / `<!-- react-craft:end -->` markers
...
- [x] **3.0.1** Each agent SKILL.md uses frontmatter: `name`, `description` (what + when), `user-invocable: false`
- [x] **3.0.2** Each agent writes standard output artifact. No separate handoff notes protocol — the artifact IS the handoff for a 4-agent linear chain. (Simplicity review: formalize handoff protocol in v0.2 when adding agents 5-7.)
- [x] **3.0.3** Each agent SKILL.md includes concrete input/output examples (create-agent-skills requirement)
- [x] **3.0.4** Agents have access to file primitives (Read, Glob, Grep) so they can pull additional context when needed (agent-native architecture recommendation)

### 3.1 Design Analyst Agent

- [x] **3.1.1** Create `skills/design-analyst/SKILL.md` + `reference.md`
  ```yaml
  ---
  name: design-analyst
  ...
  ---
  ```
- [x] **3.1.2** Input: Figma link (node-level preferred) or Anova YAML
- [x] **3.1.3** Figma URL validation:
...
- [x] **3.1.4** Cache all Figma data to `docs/react-craft/components/<ComponentName>/figma-raw.json` on first fetch. All downstream agents and remediation loops reuse cached data — never re-fetch.

- [x] **3.1.5** Process: Extract via Figma Console MCP (fallback: Official Figma MCP), parse Anova YAML if provided
- [x] **3.1.6** **Sanitize all extracted strings** through sanitization module (2.0.1) before writing to brief
- [x] **3.1.7** Wrap all Figma-sourced content in `[FIGMA_DATA]...[/FIGMA_DATA]` delimiters in the brief (anti-prompt-injection, Security NEW-1)
- [x] **3.1.8** Completeness checklist: states, responsive, motion, content rules, keyboard, tokens
- [x] **3.1.9** NEVER guess — ask human for missing info with specific questions requesting node-level Figma links
- [x] **3.1.10** Output: component brief (markdown) to `docs/react-craft/components/<ComponentName>/brief.md`
- [x] **3.1.11** Gate: block on `[PENDING]` items unless `--best-effort` is set
...
- [x] **3.2.1** Create `skills/component-architect/SKILL.md` + `reference.md`
- [x] **3.2.2** Read brief.md from Design Analyst
- [x] **3.2.3** Break complex components into atomic parts
- [x] **3.2.4** Define TypeScript prop interface:
...
- [x] **3.2.5** When React 19+ detected in config, design API with `ref` as a regular prop (no `forwardRef`). When React 18, use `forwardRef` with named function expression.

- [x] **3.2.6** Research existing libraries for complex components (Radix, React Aria, Headless UI) via web search
- [x] **3.2.7** Prefer existing DS components; check `banned_dependencies` from config before recommending new ones
- [x] **3.2.8** Output: separate `architecture.md` to component docs directory

### Research Insights: Dependency Approval (Spec Flow Analysis)

- [x] **3.2.9** When recommending a new library, present as a user gate: "I recommend @radix-ui/react-dialog. Proceed with Radix, or custom implementation?" In `--best-effort` mode, log recommendation and proceed with custom.

### 3.3 Code Writer Agent

- [x] **3.3.1** Create `skills/code-writer/SKILL.md` + `reference.md`
- [x] **3.3.2** Read brief.md + architecture.md (if architecture.md absent due to simple routing, infer structure from brief)
- [x] **3.3.3** Read config FIRST for styling, naming, token conventions, jsx_transform, path_aliases
- [x] **3.3.4** Implementation rules:
...
- [x] **3.3.5** TypeScript codegen rules:
...
- [x] **3.3.6** Faithfully translate Figma's visual character — typography, color, spacing, motion — into code. Use CSS custom properties. Don't normalize spacing/layout into generic patterns. Preserve the design's compositional choices.

- [x] **3.3.7** Output: React component file(s) + index barrel export to project source tree

### 3.4 Quality Gate Agent

- [x] **3.4.1** Create `skills/quality-gate/SKILL.md` + `reference.md`
- [x] **3.4.2** Checks (using confirmed scripts from config):
...
- [x] **3.4.3** Use temporary `tsconfig.react-craft.json` extending user's config with `include` scoped to generated files only. Reduces type-check from 30s to 3-5s on large projects.

### Research Insights: Baseline Comparison (Spec Flow Analysis)

- [x] **3.4.4** Before pipeline starts, capture baseline `tsc` error count. Quality Gate only flags NEW errors introduced by react-craft.

- [x] **3.4.5** Three-path failure handling:
...
- [x] **3.4.6** If attempt N fails with same error signature as attempt N-1, halt immediately (don't burn tokens on repeated failures)

- [x] **3.4.7** Output: quality report with PASS/FAIL per check in `review.md`

### 3.5 `/react-craft:build` Pipeline

- [x] **3.5.1** Create `commands/react-craft/build.md` with frontmatter:
  ```yaml
  ---
...
- [x] **3.5.2** Before any file generation, create a git branch: `git checkout -b react-craft/<ComponentName>`. Trivial rollback mechanism.

### Research Insights: Pre-Flight Checks (Spec Flow Analysis + Architecture Review)

- [x] **3.5.3** Pre-flight validation before pipeline starts:
...
- [x] **3.5.4** Pipeline: Design Analyst → Component Architect → Code Writer → Quality Gate
...
- [x] **3.5.5** Write `pipeline-state.yaml` tracking agent completion:
  ```yaml
  component: DataTable
  ...
  ```
- [x] **3.5.6** `--resume <ComponentName>`: read pipeline-state.yaml, resume from last failed step

### Research Insights: Artifact Integrity (Security NEW-3)

- [x] **3.5.7** Compute SHA-256 hash of each artifact after write, store in pipeline-state.yaml. Before downstream agent reads, verify hash. Halt on tamper detection.

- [x] **3.5.8** Remediation loop: if Quality Gate fails, re-invoke Code Writer with **failure diffs only** (not full report — saves 40-60% remediation tokens), max 3 attempts

### Research Insights: Remediation Terminal State (Architecture Review)

- [x] **3.5.9** When remediation exhausted (3 attempts or identical failure):
...
- [x] **3.5.10** Global iteration budget: max 6 re-invocations per pipeline run (reduced from 10 — performance review)

### Research Insights: Output Directory from Config (Architecture Review)

- [x] **3.5.11** Read `output.components_dir` and `output.docs_dir` from config for file placement. Don't hardcode paths.
...
- [x] **4.2** Document in README: "react-craft executes user-defined scripts from package.json (lint, format, typecheck). These run with your shell's permissions. Review scripts during `/react-craft:init` when prompted."

- [x] **4.3** Write CONTRIBUTING.md basics
